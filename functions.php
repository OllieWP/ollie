...
/**
 * Add a theme activation notice
 */
function display_theme_activation_notice() {
...
add_action( 'admin_notices', __NAMESPACE__ . '\display_theme_activation_notice' );


/**
 * Set a transient for the theme activation notice
 */
function dismiss_theme_activation_notice() {
	set_transient( 'ollie_activation_notice', true, YEAR_IN_SECONDS );
}

add_action( 'wp_ajax_dismiss_theme_activation_notice', __NAMESPACE__ . '\dismiss_theme_activation_notice' );
...
function enqueue_notice_script() {
	...
	if ( $current_screen && $current_screen->base === 'themes' ) {
